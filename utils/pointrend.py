...
def get_class_masks_from_instances(
    ...
    keep_annotations = []
    keep_masks = []
    # instances.image_size: in H, W notation
    full_boxes = torch.tensor([[0, 0, instances.image_size[1], instances.image_size[0]]] * len(boxes)).float()
    full_sized_masks = bit_masks.crop_and_resize(full_boxes, rend_size)
